...
class InventorySync extends EventEmitter {
  ...
  constructor () {
    ...
    async.series([
      ...
      (done) => {
        _broker.createRPC('client', 'plugin.state.rpc').then((queue) => {
    ...
    this.setState = (state) => {
      return new BPromise((resolve, reject) => {
        if (isEmpty(state)) return reject(new Error(`Please specify a state to set.`))

        _broker.queues['plugin.state'].publish({
          state: state,
          plugin: PLUGIN_ID
