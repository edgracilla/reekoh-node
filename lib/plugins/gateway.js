...
class Gateway extends EventEmitter {
  ...
  constructor () {
    ...
    const COMMAND_RELAY = process.env.COMMAND_RELAY
    ...
    process.env.COMMAND_RELAY = undefined
    ...
    async.series([
      ...
      (done) => {
        let queueIds = [PLUGIN_ID, COMMAND_RELAY, 'cmd.responses', 'devices', 'data', 'logs', 'exceptions']
      ...
      (done) => {
        _broker.createExchange(`${COMMAND_RELAY}.topic`).then((exchange) => {
    ...
    this.relayCommand = (command, devices, deviceGroups) => {
      return new BPromise((resolve, reject) => {
        ...
        _broker.queues[COMMAND_RELAY].publish({
          command: command,
