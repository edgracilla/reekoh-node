...
class Gateway extends EventEmitter {
  ...
  constructor () {
    ...
    async.series([
      ...
      (done) => {
        async.each(COMMAND_RELAYS, (cmdRelay, cb) => {
          _broker.createExchange(`${cmdRelay}.topic`).then((exchange) => {
            return exchange.consume((message) => {
              if (!isEmpty(message)) {
                async.waterfall([
                  async.constant(message.content.toString('utf8')),
                  async.asyncify(JSON.parse)
                ], (err, msg) => {
                  if (err) return console.error('Gateway exchange.consume() rcvd data is not a JSON.', err)

                  if (!_cmdCache.get(msg.sequenceId)) {
                    _cmdCache.set(msg.sequenceId, '0')

                    delete msg.sequenceId
      ...
      (done) => {
        _broker.createRPC('client', 'cmd.pending.rpc').then((queue) => {
          return queue.consume()
        }).then(() => {
          return done() || null
        }).catch(done)
    ...
    this.relayCommand = (command, devices, deviceGroups, source = '') => {
      return new BPromise((resolve, reject) => {
        ...
        async.each(COMMAND_RELAYS, (cmdRelay, cb) => {
          _broker.queues[cmdRelay].publish({
            command: command,
            sequenceId: sequenceId,
            deviceGroups: deviceGroups,
            devices: devices,
            source: source
          }).then(cb).catch(reject)
        }, (err) => {
          if (err) return reject(err)
          resolve()
        })
    ...
    this.notifyConnection = (deviceId) => {
      return new BPromise((resolve, reject) => {
        ...
        _broker.queues['devices'].publish({
          ...
          _broker.rpcs['cmd.pending.rpc'].once(requestId, (commands) => {
            if (!hasTimedOut) {
              clearTimeout(t)

              async.waterfall([
                async.constant(commands || '{}'),
                async.asyncify(JSON.parse)
              ], (err, arrCmd) => {
                if (err) return reject(err)

                async.each(arrCmd, (command, done) => {
                  async.waterfall([
                    async.constant(command || '{}'),
                    async.asyncify(JSON.parse)
                  ], (err, cmd) => {
                    if (err) return reject(err)
          ...
          return _broker.rpcs['cmd.pending.rpc'].publish(requestId, {
            account: ACCOUNT,
            device: {
              _id: deviceId,
              requestId: requestId,
