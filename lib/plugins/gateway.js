...
class Gateway extends EventEmitter {
  ...
  constructor () {
    ...
    async.series([
      ...
      (done) => {
        _broker.createRPC('client', 'pendingcommand').then((queue) => {
          return queue.consume()
        }).then(() => {
          return done() || null
        }).catch(done)
    ...
    this.notifyConnection = (deviceId) => {
      return new BPromise((resolve, reject) => {
        ...
        _broker.queues['devices'].publish({
          ...
          _broker.rpcs['pendingcommand'].once(requestId, (commands) => {
            if (!hasTimedOut) {
              clearTimeout(t)
          ...
          return _broker.rpcs['pendingcommand'].publish(requestId, {
            account: ACCOUNT,
            device: {
              _id: deviceId,
              requestId: requestId
